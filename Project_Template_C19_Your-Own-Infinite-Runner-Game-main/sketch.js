...
function draw() {
    ...
    if (gameState === "end"){
        bird.addAnimation("stopped", birdStopImg)
        bird.velocityY = 0
...
function spawnObstacles() {
    if (frameCount % 60 === 0){
        var tornado = createSprite(windowWidth/2,windowHeight/2)
        tornado.addImage(tornadoImg)
        tornado.y = Math.round(random(windowWidth-50,50))
        tornado.velocityY = -4
        tornado.depth = background1.depth
        tornado.depth+=1
    }
}